...
else if($dopost == 'moveCatalog')
{
    ...
    else
    {
		...
		/*修正TOPID*/
		$tl = new TypeLink($movetype);
		$topid = $tl->TypeInfos['topid'];
		if($topid==0){
			$topid = $movetype;
		}
...
/*----------------
批量移动栏目
function batchMoveCatalog() { }
-----------------*/
else if($dopost == 'batchMoveCatalog')
{
    CheckPurview('t_Move');
    require_once(DEDEINC.'/oxwindow.class.php');
    require_once(DEDEINC.'/typelink.class.php');
	...
	if(empty($tids))
	{
		 ShowMsg('请选择需要移动的栏目！', 'catalog_main.php');
		 exit();
	}
	...
	if(empty($nextjob))
	{
		if(!is_array($tids))
		{
			 ShowMsg('移动栏目数据必须是数组！', 'catalog_main.php');
			 exit();
		}
		
		$tl = new TypeLink($tids[0]);
		$channelid = $tl->TypeInfos['channeltype'];
		$typeOptions = $tl->GetOptionArray(0,0,$channelid);
		$wintitle = "移动栏目";
		$wecome_info = "<a href='catalog_main.php'>栏目管理</a> &gt;&gt; 移动栏目";
		$win = new OxWindow();
		$win->Init('catalog_do.php', 'js/blank.js', 'POST');
		$win->AddHidden('dopost', 'batchMoveCatalog');
		$win->AddHidden('tids', implode(',', $tids));
		$win->AddHidden('nextjob', 'unitok');
		$win->AddTitle("移动目录时不会删除原来已创建的列表，移动后需重新对栏目创建HTML。");
		$win->AddItem('你选择的栏目是：',implode(',', $tids));
		$win->AddItem('你希望移动到那个栏目？',"<select name='movetype'>\r\n<option value='0'>移动为顶级栏目</option>\r\n$typeOptions\r\n</select>");
		$win->AddItem('注意事项：','不允许从父级移动到子级目录，只允许子级到更高级或同级或不同父级的情况。');
		$winform = $win->GetWindow('ok');
		$win->Display();
	...
	else
	{
		...
		foreach($arr as $tid)
		{
			if($tid==$movetype)
			{
				ShowMsg("移对对象[{$tid}]和目标位置[{$movetype}]相同！", 'catalog_main.php');
				exit();
			}
			if(IsParent($movetype, $tid))
			{
				ShowMsg("不能从父类[{$tid}]移动到子类[{$movetype}]！", 'catalog_main.php');
				exit();
			}
		}
		...
		$dsql->ExecuteNoneQuery(" UPDATE `#@__arctype` SET reid='$movetype', topid='$topid' WHERE id IN ({$tids}) ");
		UpDateCatCache();
		ShowMsg('成功移动目录！', 'catalog_main.php');
		exit();
	}
