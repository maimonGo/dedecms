        ...
        $innertext='', $arcid=0, $idlist='', $channelid=0, $limit='', $att='', $order='desc', $subday=0, $noflag='',$tagid='', $pagesize=0, $isweight='N')
{
	 ...
	 * 获取当前文章定义样式
	 */
	$currentclass = $ctag->GetAtt('currentclass');
	/*=========================================*/
    ...
    for($i=0; $i<$line; $i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row = $dsql->GetArray("al"))
            {
				 ...
				 */
				$arcsel = FALSE;
				$items = $ctag->CAttribute->Items; 
				if($row['id'] == $arcid){
					$arcsel = TRUE;
				}elseif(isset($refObj->ChannelUnit)){
					if(isset($refObj->ChannelUnit->ArcID)){
						if($row['id'] == $refObj->ChannelUnit->ArcID){
							$arcsel = TRUE;
						}
					}
				}
