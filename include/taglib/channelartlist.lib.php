...
function lib_channelartlist(&$ctag,&$refObj)
{
    ...
    //处理标记属性、innertext
    $attlist = 'typeid|0,row|20,cacheid|,currentclass|';
    ...
    else
    {
        if(!preg_match('#,#', $typeid)) 
		{
			 ...
			 * 获取第二层栏目
			 */
			if(strtolower(trim($twolevel))=='yes')
			{
				$topid = ( !empty($refObj->TypeLink->TypeInfos['topid']) ?  $refObj->TypeLink->TypeInfos['topid'] : $refObj->TypeLink->TypeInfos['id'] );
				$tpsql = " reid='$topid' AND ispart<>2 AND ishidden<>1 ";
			}else{
    ...
    for($i=0;isset($typeids[$i]);$i++)
    {
		 ...
		 * 当前栏目Class
		 */
		if($typeids[$i]['id'] == $refObj->TypeLink->TypeInfos['id'] || $typeids[$i]['id'] == $refObj->TypeLink->TypeInfos['reid'] || $typeids[$i]['id'] == $refObj->TypeLink->TypeInfos['topid'] ){  
        	$pv->Fields['currentclass'] = empty($currentclass) ? 'active' : $currentclass;  
        }else{  
            $pv->Fields['currentclass'] = '';  
        }
		
		$tsql = "SELECT id From `#@__arctype` WHERE reid='{$typeids[$i]['id']}' And ishidden<>1 order by sortrank asc limit 1";
		$trow = $dsql->GetOne($tsql); 
		$pv->Fields['haschild'] = is_array($trow) ? 1 : 0; 
		/*=========================================*/
