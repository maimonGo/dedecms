...
function lib_channel(&$ctag,&$refObj)
{
	 ...
	 * currentclass 当前栏目样式
	 */
    $attlist = "typeid|0,reid|0,row|100,col|1,type|son,currentstyle|,cacheid|,showhide|,currentclass|";
	/*=========================================*/
	 ...
	 * 判断是否显示隐藏栏目
	 */
	$and_ishide = ($showhide=='yes') ? '' : 'And ishidden<>1';
	/*=========================================*/

    if($type=='top')
    {
        $sql = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
          From `#@__arctype` WHERE reid=0 $and_ishide order by sortrank asc limit 0, $line ";
    }
    else if($type=='son')
    {
        if($typeid==0) return '';
        $sql = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
          From `#@__arctype` WHERE reid='$typeid' $and_ishide order by sortrank asc limit 0, $line ";
    }
    else if($type=='self')
    {
        if($reid==0) return '';
        $sql = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
            FROM `#@__arctype` WHERE reid='$reid' $and_ishide order by sortrank asc limit 0, $line ";
    }
    //And id<>'$typeid'
    $needRel = false;
            ...
            if($row=$dsql->GetArray())
            {
                ...
                else
                {
					...
					if( ($row['id']==$typeid || ($topid==$row['id'] && $type=='top') )){
						$row['currentclass'] = empty($currentclass) ? 'active' : $currentclass;
                	}else{
						$row['currentclass'] = '';
					}
					/*=========================================*/
